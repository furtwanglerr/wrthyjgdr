...
module.exports = {
  name: "play",
  description: "음악재생 명령어",
  permissions: "0x0000000000000800",
  options: [{
    name: 'name',
    description: '재생할 음악 제목을 적으세요',
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name) return interaction.reply({ content: `❌ 유효한 음악 제목을 적으세요`, ephemeral: true }).catch(e => { });
      let res;
      try {
        ...
        return interaction.editReply({ content: `❌ 결과없음` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ 결과없음`, ephemeral: true }).catch(e => { });
